...
const Schedule = ({ onAnimeClick }) => {
  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      setLoading(true);
      try {
        const data = await fetchAPI('/anime/schedule/week');
        if (data) {
          setSchedule(data);
        }
      } catch (err) {
  ...
  const getAnimeListForDay = (day) => {
    ...
    if (['today', 'tomorrow', 'yesterday'].includes(day)) {
      return schedule[day]?.map(item => item.release) || [];
    }

    const numericDay = parseInt(day, 10);
    if (!isNaN(numericDay) && schedule?.days && Array.isArray(schedule.days)) {
      return schedule.days[numericDay]?.map(item => item.release) || [];
    }
          ...
          >
            <motion.button
              ...
              onClick={() => {
                ...
                if (showFullSchedule && !['today', 'tomorrow', 'yesterday'].includes(activeDay)) {
                  setActiveDay('today');
                }
              }}
