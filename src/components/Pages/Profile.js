...
import { fetchUserInfo, logout, fetchCurrentUser, fetchWithAuth } from '../../utils/external/shikimori/shikimoriAuth';
import { getUserAnimeRates, getStatusDisplayName } from '../../utils/external/shikimori/shikimoriRates';
import Login from '../Auth/Login';
import { PieChart, Pie, Cell, ResponsiveContainer, BarChart, Bar, XAxis, YAxis, Tooltip } from 'recharts';
import { fetchAPI } from '../../utils/external/api';

import './styles/profile.css';
import '../../styles/perfs/sortDropDown.css';
...
const Profile = ({ onAnimeClick, navigateTo }) => {
  ...
  // Filter and sort user rates based on search query and sort option
  useEffect(() => {
    ...
    let filtered = [...userRates];

    // Apply search filter
    if (searchQuery.trim() !== '') {
      const query = searchQuery.toLowerCase().trim();
      filtered = filtered.filter(rate => 
        rate.anime.name.toLowerCase().includes(query) || 
        (rate.anime.name_eng && rate.anime.name_eng.toLowerCase().includes(query))
      );
    }
    ...
    switch (sortOption) {
      case 'name-asc':
        filtered.sort((a, b) => (a.anime.name || '').localeCompare(b.anime.name || ''));
        break;
      case 'name-desc':
        filtered.sort((a, b) => (b.anime.name || '').localeCompare(a.anime.name || ''));
        break;
      case 'score-desc':
      ...
      case 'date-desc':
        filtered.sort((a, b) => {
          const dateA = a.anime.aired_on ? new Date(a.anime.aired_on) : new Date(0);
          const dateB = b.anime.aired_on ? new Date(b.anime.aired_on) : new Date(0);
          return dateB - dateA;
        });
        break;
      case 'date-asc':
      ...
      case 'episodes-desc':
        ...
        break;
      case 'episodes-asc':
        filtered.sort((a, b) => (a.episodes || 0) - (b.episodes || 0));
        break;
      default:
        break;
    }
  ...
  }, [userRates, searchQuery, sortOption]);
                  ...
                  {activeTab === 'anime-list' && (
                    ...
                    >
                      <div className="anime-list-container">
                        ...
                        <motion.div 
                          className="anime-list-search-sort"
                          initial={{ opacity: 0, y: 20 }}
                          animate={{ opacity: 1, y: 0 }}
                          transition={{ duration: 0.3 }}
                          style={{ 
                        ...
                        >
                          <div className="business-search">
                            <div className="business-search-label">Поиск</div>
                          ...
                          <div className="business-select">
                            <div className="business-select-label">Сортировка</div>
                            <select 
                              value={sortOption} 
                            ...
                            >
                              ...
                              <option value="score-desc">По оценке ↓</option>
                              <option value="score-asc">По оценке ↑</option>
                              <option value="date-desc">По дате выхода ↓</option>
                              <option value="date-asc">По дате выхода ↑</option>
                        ...
                        <div className="anime-list-content">
                          {loadingList ? (
                                    ...
                                    {filteredRates.map((rate, index) => (
                                      ...
                                      >
                                        ...
                                        <td className="anime-table-score">
                                          {rate.score > 0 ? (
                                            <div 
                                              className="anime-score-value" 
                                              style={{ 
                                                color: SCORE_COLORS[rate.score - 1],
                                                display: 'flex',
                                                alignItems: 'center',
                                                gap: '4px'
                                              }}
                            ...
                            <EmptyState
                              ...
                              title={`Ничего не найдено по запросу "${searchQuery}"`}
                              message="Попробуйте изменить поисковый запрос или очистить фильтр"
                            />
                          ) : (
                            <EmptyState
                              icon={
